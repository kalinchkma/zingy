import type { Contact, Footer, Header, Settings } from '../../payload/payload-types'
import { CONTACT_QUERY, FOOTER_QUERY, HEADER_QUERY, SETTINGS_QUERY } from '../_graphql/globals'
...
export async function fetchContact(): Promise<Contact> {
  ...
  const contact = await fetch(`${GRAPHQL_API_URL}/api/graphql`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
