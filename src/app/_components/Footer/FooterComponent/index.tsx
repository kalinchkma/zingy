      ...
      <div className={classes.footer}>
        <Gutter>
          ...
          <div className={cn(classes.contact_wrap, 'md:col-span-2')}>
            ...
            <div className={cn('flex w-full flex-col', classes.contactAddress)}>
              {contactAddresses.map((address, i) => (
                <p key={i} className={cn(classes.footerText, 'break-words')}>{address.address}</p>
              ))}
            </div>
            {/* social links */}
            ...
            navItems.map((items, i) => (
              <div key={i}>
                  ...
                  <ul className='flex flex-col gap-2 '>
                    {
                      items.navCollections.navGroup.map((navItem, i) => (
                        ...
                        <li key={i}>
                          <CMSLink className={cn(classes.footerLink, classes.footerText )} {...navItem.link} />
                        </li>
                      ))
                    }
                  </ul>
                ...
                <Collapsible className='block md:hidden'>
                  <CollapsibleTrigger className={cn(classes.footerGap, 'w-full text-lg font-bold m-0 py-3 flex flex-row justify-between items-center border-b-2 border-stone-400 mb-4')}>
                    <span>{items.navCollections.title} </span><FaPlus />
                    </CollapsibleTrigger>
                  <CollapsibleContent>
          ...
          <div className={cn('flex flex-col md:col-span-3 lg:col-span-2', classes.newslatter)}>
            ...
            <p className={cn('text-sm', classes.footerText)}>Receive our latest updates about our products and promotions.</p>
            <NewsletterForm />
          </div>
          </div>
        </Gutter>
