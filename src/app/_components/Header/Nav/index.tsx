...
export const HeaderNav: React.FC<{ header: HeaderType }> = ({ header }) => {
  ...
  return (
    <div className={[classes.nav, user === undefined && classes.hide, 'py-8'].filter(Boolean).join(' ')}>
      <Menu setActive={setActive}>
        {
          ...
          <MenuItem setActive={setActive} active={active} key={i} item={navGroup.link.label} href={navGroup.link?.url} toltip={navGroup.link.toltip}>
            {
      ...
      {!user && (
        <>
          <Link href={'/login'} onClick={() => (window.location.href = '/login')} className='text-sm transition-all duration-500 font-bold'>
            Sign In
          </Link>
        </>
        )}
        <SearchWebsite />
        {user && <Link href="/account" className='font-bold'>
